...
import { AttachmentLinks } from './AttachmentPicker';
...
export const BookSalesView: React.FC<BookSalesViewProps> = ({
  ...
  const { saleRecords, deleteBookSale } = useApp();
  ...
  const [saleToDelete, setSaleToDelete] = useState<BookSaleRecord | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            <h2 className="text-xl font-bold text-slate-900 font-serif">Book Sales & Invoices</h2>
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex items-center space-x-1 bg-slate-100 p-1 rounded-xl text-xs">
          {(['All', 'Paid', 'Payment Remaining'] as const).map((st) => (
            ...
            >
              {st === 'Payment Remaining' ? 'Unpaid' : st}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Sale Detail Popup */}
      {detailSaleModal && (
        <Modal isOpen onClose={() => setDetailSaleModal(null)} maxWidth="max-w-lg">
          <ModalHeader onClose={() => setDetailSaleModal(null)}>
            <div className="min-w-0">
              ...
              <h3 className="text-lg sm:text-xl font-bold text-slate-900 font-serif mt-1">{detailSaleModal.customerName}</h3>
                ...
                <div className="mt-3">
                  ...
                  <div className="space-y-1">
                    {detailSaleModal.paymentHistory.map((p) => (
                      <div key={p.id} className="p-2 bg-slate-50 rounded-lg text-[11px]">
                        ...
                        <AttachmentLinks files={p.attachments} />
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <ModalFooter>
            ...
            <button
              ...
              className="w-full sm:w-auto px-3 py-2.5 bg-white border border-slate-200 text-slate-700 font-semibold text-xs rounded-xl"
            >
              Edit Sale
            </button>
            <button
              ...
              className="w-full sm:w-auto px-3 py-2.5 bg-slate-800 text-white font-semibold text-xs rounded-xl"
            >
              Print Receipt
            </button>
          </ModalFooter>
      ...
      {saleToDelete && (
        <Modal isOpen onClose={() => setSaleToDelete(null)} maxWidth="max-w-md">
          <ModalHeader onClose={() => setSaleToDelete(null)}>
            <div className="flex items-center space-x-2 text-rose-700">
              <Trash2 className="w-5 h-5 shrink-0" />
              <h3 className="text-base sm:text-lg font-bold">Delete Sale Invoice</h3>
            </div>
          </ModalHeader>
          <ModalBody>
            <p className="text-sm text-slate-600">
              Delete invoice <strong>{saleToDelete.invoiceNo}</strong> for{' '}
              <strong>{saleToDelete.customerName}</strong>? The sold copies will be added back to stock.
            </p>
          </ModalBody>
          <ModalFooter>
            <button
            ...
            <button
              ...
              onClick={() => {
                deleteBookSale(saleToDelete.id);
                setSaleToDelete(null);
              }}
              className="w-full sm:w-auto px-4 py-2.5 text-sm font-medium text-white bg-rose-600 hover:bg-rose-700 rounded-lg"
            >
              Delete Invoice
            </button>
          </ModalFooter>
