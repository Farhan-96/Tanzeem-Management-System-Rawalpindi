...
import { OfficeAsset, AssetStatus, AssetCategory, StoredAttachment } from '../types';
...
export const OfficeAssetsView: React.FC<OfficeAssetsViewProps> = ({ openAddAssetModal }) => {
  const { assets, updateAssetStatus, assignAsset, deleteAsset, updateAsset, currentUser } = useApp();
  if (!currentUser) return null;
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<string>('All');
  const [categoryFilter, setCategoryFilter] = useState<string>('All');
  const [assignModalAsset, setAssignModalAsset] = useState<OfficeAsset | null>(null);
  const [filesModalAsset, setFilesModalAsset] = useState<OfficeAsset | null>(null);
  const [assetFiles, setAssetFiles] = useState<StoredAttachment[]>([]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            <h2 className="text-xl font-bold text-slate-900 font-serif">Office Assets & Equipment</h2>
            ...
            >
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-1">
                <div className="flex items-center space-x-1">
                  ...
                  <button
                    id={`asset-files-${asset.id}`}
                    onClick={() => {
                      setFilesModalAsset(asset);
                      setAssetFiles(asset.attachments || []);
                    }}
                    className="px-2.5 py-1.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 rounded-lg text-xs font-semibold flex items-center space-x-1"
                  >
                    <Paperclip className="w-3.5 h-3.5" />
                    <span>Documents{asset.attachments?.length ? ` (${asset.attachments.length})` : ''}</span>
                  </button>
      ...
      {/* Assign Person Modal */}
      {assignModalAsset && (
        <Modal isOpen onClose={() => setAssignModalAsset(null)} maxWidth="max-w-md">
          <ModalHeader onClose={() => setAssignModalAsset(null)}>
            <div className="min-w-0">
              <h3 className="text-base sm:text-lg font-bold text-slate-900 font-serif">
                Assign to Staff
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">Asset: {assignModalAsset.name} [{assignModalAsset.assetTag}]</p>
            </div>
          </ModalHeader>
          <form onSubmit={handleAssignSubmit} className="flex flex-col flex-1 min-h-0">
            ...
            <ModalFooter>
              ...
              <button
                type="submit"
                className="w-full sm:w-auto px-4 py-2.5 bg-teal-700 text-white font-bold text-xs rounded-xl hover:bg-teal-800"
              >
                Save Assignment
              </button>
            </ModalFooter>
      ...
      {filesModalAsset && (
        <Modal isOpen onClose={() => setFilesModalAsset(null)} maxWidth="max-w-md">
          <ModalHeader onClose={() => setFilesModalAsset(null)}>
            <div className="min-w-0">
              <h3 className="text-base sm:text-lg font-bold text-slate-900 font-serif">Asset Documents</h3>
              <p className="text-xs text-slate-500 mt-0.5">
                {filesModalAsset.name} [{filesModalAsset.assetTag}]
              </p>
            </div>
          </ModalHeader>
          <ModalBody className="space-y-3">
            <AttachmentLinks files={assetFiles} />
            <AttachmentPicker
              label="Add PDF or image"
              hint="Purchase bills, warranty papers, photos, or other records."
              kind="asset-document"
              value={assetFiles}
          ...
          <ModalFooter>
            <button
              type="button"
              onClick={() => setFilesModalAsset(null)}
              className="w-full sm:w-auto px-4 py-2.5 bg-slate-100 text-slate-600 font-medium text-xs rounded-xl"
            >
              Close
            </button>
            <button
              type="button"
              onClick={() => {
                updateAsset({ ...filesModalAsset, attachments: assetFiles });
                setFilesModalAsset(null);
              }}
              className="w-full sm:w-auto px-4 py-2.5 bg-teal-700 text-white font-bold text-xs rounded-xl hover:bg-teal-800"
            >
              Save Documents
